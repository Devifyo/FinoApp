    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="group relative flex flex-col h-full bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/30 border-t border-gray-100 dark:border-gray-700">
                <div class="text-xs font-semibold text-gray-400 uppercase mb-2">Team Allocation</div>
                <div class="flex flex-wrap gap-2">
                    @foreach($project->members as $member)
                        <div class="inline-flex items-center bg-white dark:bg-gray-700 rounded-full px-2.5 py-1 border border-gray-200 dark:border-gray-600 shadow-sm">
                             <img class="h-5 w-5 rounded-full mr-2" src="https://ui-avatars.com/api/?name={{ $member->user->name }}&background=random&size=32" alt=""/>
                             <span class="text-xs font-medium text-gray-700 dark:text-gray-200 mr-1.5">{{ $member->user->name }}</span>
                             <span class="text-[10px] font-bold px-1.5 py-0.5 rounded-md 
                                {{ $member->role == 'bidder' ? 'bg-blue-100 text-blue-700' : 
                                  ($member->role == 'developer' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600') }}">
